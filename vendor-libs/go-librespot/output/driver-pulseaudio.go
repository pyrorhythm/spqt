...
// findPulseSocket detects the PulseAudio unix socket path on macOS.
// Homebrew PulseAudio names its runtime dir with a machine-id hash rather than
// the hostname, so the jfreymuth/pulse default path detection fails on macOS.
func findPulseSocket() string {
	if runtime.GOOS != "darwin" {
		return ""
	}

	dir := filepath.Join(os.Getenv("HOME"), ".config", "pulse")
	entries, err := os.ReadDir(dir)
	if err != nil {
		return ""
	}
	for _, e := range entries {
		if !e.IsDir() || !strings.HasSuffix(e.Name(), "-runtime") {
			continue
		}
		sock := filepath.Join(dir, e.Name(), "native")
...
func newPulseAudioOutput(opts *NewOutputOptions) (*pulseAudioOutput, error) {
	...
	clientOpts := []pulse.ClientOption{
		pulse.ClientApplicationName("go-librespot"),
		pulse.ClientApplicationIconName("speaker"),
	}
	if sock := os.Getenv("PA_SOCKET"); sock != "" {
		clientOpts = append(clientOpts, pulse.ClientServerString(sock))
	}
